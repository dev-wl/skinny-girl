...
class ET_Monarch {
	...
	function __construct() {
		...
		add_action( 'init', array( $this, 'include_options' ) );
	}
	...
	function display_notices( $notices, $screen ) {
		foreach ( $notices as $notice ) {
			$notice_link = sprintf(
				'<a href="%1$s%2$s" class="et_social_notice_link">%3$s.</a>',
				esc_url( admin_url( "tools.php?page=et_monarch_options#tab_et_social_tab_content_" ) ),
				esc_attr( $notice['tab'] ),
				__( 'here', 'Monarch' )
			);
	...
	function maybe_display_notices() {
		...
		if ( ( in_array( $screen->base, array( 'tools_page_et_monarch_options', 'plugins' ) ) ) && ( ! isset( $monarch_options['ignore_monarch_fb_notice'] ) ) ) {
			$notices[] = array(
				'Network'     => 'Facebook',
				'network'     => 'fb',
				'counts'      => __( 'follow and share', 'Monarch' ),
				'credentials' => __( 'an App ID and App Secret', 'Monarch' ),
				'tab'         => 'general_main',
			);
		}
		...
		if ( ( in_array( $screen->base, array( 'tools_page_et_monarch_options', 'plugins' ) ) ) && ( ! isset( $monarch_options['ignore_monarch_youtube_notice'] ) ) ) {
			$notices[] = array(
				'Network'     => 'Youtube',
				'network'     => 'youtube',
				'counts'      => __( 'subscriber', 'Monarch' ),
				'credentials' => __( 'an API Key', 'Monarch' ),
				'tab'         => 'follow_networks',
			);
		}
	...
	function ignore_notice() {
		if ( isset( $_GET['ignore_monarch_notice'] ) && '1' === $_GET['ignore_monarch_notice'] ) {
			$network = in_array( $_GET['network'], array( 'fb', 'youtube' ) ) ? $_GET['network'] : '';
			if ( '' !== $network ) {
	...
	function upgrade_plugin() {
		...
		if ( version_compare( $monarch_options['db_version'], '1.2', '<' ) ) {
	...
	function add_stats_record_db() {
		...
		$result = $this->add_stats_row( $stats_data_array[ 'network' ], $stats_data_array[ 'post_id' ], $_SERVER[ 'REMOTE_ADDR' ], '', $stats_data_array[ 'action' ], $stats_data_array[ 'media_url' ], $stats_data_array[ 'location' ] );
										...
										foreach( $value[ 'contents' ] as $key => $value ) {
											...
											printf( '<div class="et_social_tab_content et_social_tab_content_%1$s_%2$s">',
												...
												foreach( $options_array as $option) {
													...
													case 'checkbox' :
														printf( '<li class="et_social_checkbox clearfix%5$s%7$s"%4$s%6$s><p>%1$s</p><input type="checkbox" id="et_social[%2$s]" name="et_social[%2$s]" value="1" %3$s><label for="et_social[%2$s]"></label>',
																isset( $option['title_' . $current_location] ) ? esc_html( $option['title_' . $current_location] ) : esc_html( $option['title'] ),
																esc_attr( $current_option_name ),
														...
														printf(
															'%5$s<div class="et_social_form et_social_row%2$s"%3$s%4$s>
																...
																( isset( $option[ 'sub_section' ] ) && true == $option[ 'sub_section' ] )
																	? sprintf( '<li class="et_social_auto_height%1$s">', isset( $option[ 'class' ] ) ? ' ' . esc_attr( $option[ 'class' ] ) : ''  )
																	: '',
	...
	function generate_stats_summary( $location = 'all', $close_div = 'close' ) {
		$output = sprintf(
			'<div id="et_social_globalstats" class="et_social_row">
				<div class="et_social_globalstat et_social_globalstat_shares"><span>%1$s</span><p><span class="et_social_globalstat_label">%2$s</span></p></div>
				<div class="et_social_globalstat et_social_globalstat_likes"><span>%3$s</span><p><span class="et_social_globalstat_label">%4$s</span></p></div>
				<div class="et_social_globalstat et_social_globalstat_follows"><span>%5$s</span><p><span class="et_social_globalstat_label">%6$s</span></p></div>
			</div>
	...
	*/
	public static function get_full_number( $compact_number ) {
		...
		if ( false !== strrpos( $compact_number, __( 'k', 'Monarch' ) ) ) {
			$compact_number = floatval( str_replace( __( 'k', 'Monarch' ), '', $compact_number ) ) * 1000;
		}
		if ( false !== strrpos( $compact_number, __( 'Mil', 'Monarch' ) ) ) {
			$compact_number = floatval( str_replace( __( 'Mil', 'Monarch' ), '', $compact_number ) ) * 1000000;
		}
			...
			switch ( $social_network ) {
				case 'facebook' :
					if ( isset( $monarch_options['access_tokens']['facebook'] ) ) {
						$request_url = sprintf( 'https://graph.facebook.com/v2.4/?access_token=%1$s&id=', $monarch_options['access_tokens']['facebook'] );
					}
			...
			foreach ( $current_networks as $icon ) {
				...
				if ( !( 'media' == $type && 'like' == $icon ) ) {
					switch ( $type ) {
						...
						case 'inline' :
							$sharing_icons .= sprintf(
								'<li class="et_social_%1$s"><a href="%3$s" class="et_social_share%6$s%7$s%9$s" rel="nofollow" data-social_name="%1$s" data-post_id="%4$s" data-social_type="%5$s" data-location="%10$s"%8$s><i class="et_social_icon et_social_icon_%1$s"></i>%2$s<span class="et_social_overlay"></span></a></li>',
									esc_attr( $icon ),
									$icon_label,
