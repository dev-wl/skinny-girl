...
class ET_Bloom extends ET_Dashboard {
	...
	function __construct() {
		...
		add_action( 'plugins_loaded', array( $this, 'construct_dashboard' ) );
...
<?php
	...
	function authorize_account() {
		...
		if ( 'true' == $update_existing ) {
			...
			$organization_id = ! empty( $accounts_data[$service][$name]['organization_id'] ) ? $accounts_data[$service][$name]['organization_id'] : '';
		...
		if ( $provider ) {
			...
			foreach ( $provider->get_fields() as $field_name => $field ) {
				if ( 'true' == $update_existing ) {
					$field_value = ! empty( $accounts_data[ $service ][ $name ][ $field_name ] ) ? $accounts_data[ $service ][ $name ][ $field_name ] : '';
				} else {
	...
	function subscribe_infusionsoft( $api_key, $app_id, $list_id, $email, $name = '', $last_name = '' ) {
		...
		if ( empty( $error_message ) ) {
			...
			if ( 0 < count( $found_contact ) ) {
				...
				$infusion_app->opt_in_email( $email, __( 'Subscribed via Bloom', 'bloom' ) );
	...
	function save_salesforce_organization_data( $organization_id, $name ) {
		...
		return __( 'Organization ID cannot be empty', 'bloom' );
	}
	...
	function subscribe_salesforce( $account_name = '', $email, $name = '', $last_name = '' ) {
		if ( '' === $account_name ) {
			return __( 'Unknown Organization ID', 'bloom' );
		}
	...
	function subscribe_hubspot( $api_key, $list_id, $email, $name = '', $last_name = '' ) {
		...
		if ( '' === $new_contact_vid ) {
			return __( 'You were not subscribed. Please try again later', 'bloom' );
		}
	...
	function subscribe_constant_contact( $email, $api_key, $token, $list_id, $name = '', $last_name = '' ) {
		...
		if ( ! is_wp_error( $theme_request ) && $response_code == 200 ){
			...
			if ( ! empty( $response['results'][0]['lists'] ) || ( isset( $response['results'][0]['status'] ) && 'REMOVED' === $response['results'][0]['status'] ) ) {
				...
				$request_url = esc_url_raw( 'https://api.constantcontact.com/v2/contacts/' . $contact_id . '?api_key=' . $api_key );
		...
		if ( ! empty( $optins_set ) ) {
			foreach( $optins_set as $optin_id => $details ) {
				if ( $this->check_applicability( $optin_id ) ) {
					...
					printf(
						'<div class="et_bloom_flyin et_bloom_optin et_bloom_resize et_bloom_flyin_%6$s et_bloom_%5$s%17$s%1$s%2$s%18$s%19$s%20$s%22$s%27$s"%3$s%4$s%16$s%21$s%26$s>
						...
						isset( $details['trigger_click'] ) && $details['trigger_click']
							...
							: '',
						isset( $details['trigger_click'] ) && $details['trigger_click'] ? ' et_bloom_trigger_click' : '' //#27
					);
				}
			}
		}
		...
		if ( ! empty( $optins_set ) ) {
			foreach( $optins_set as $optin_id => $details ) {
				if ( $this->check_applicability( $optin_id ) ) {
					...
					printf(
						'<div class="et_bloom_popup et_bloom_optin et_bloom_resize et_bloom_%5$s%15$s%1$s%2$s%16$s%17$s%18$s%20$s%22$s"%3$s%4$s%14$s%19$s%21$s>
						...
						( isset( $details['hide_mobile_optin'] ) && true == $details['hide_mobile_optin'] )
							? ' et_bloom_hide_mobile_optin'
							: '',
						true == $details['trigger_click']
							? ' data-trigger_click="' . esc_attr( $details['trigger_click_selector'] ) . '"'
							: '',
						isset( $details['trigger_click'] ) && true == $details['trigger_click'] ? ' et_bloom_trigger_click' : '' //#22
					);
				}
			}
		}
...
class Bloom_ActiveCampaign extends ET_Bloom_Email_Provider {
	function init() {
		$this->name = __( 'ActiveCampaign', 'bloom' );
	...
	function get_fields() {
		$fields = array(
			'api_url' => array(
				'label' => __( 'API URL', 'bloom' ),
			),
			'api_key' => array(
				'label' => __( 'API Key', 'bloom' ),
			),
			'form_id' => array(
				'label'               => __( 'Form ID', 'bloom' ),
				'not_required'        => true,
...
class Bloom_Emma extends ET_Bloom_Email_Provider {
	...
	function get_fields() {
		$fields = array(
			'api_key' => array(
				'label' => __( 'Public API Key', 'bloom' ),
			),
			'private_api_key' => array(
				'label' => __( 'Private API Key', 'bloom' ),
			),
			'user_id' => array(
				'label' => __( 'Account ID', 'bloom' ),
			),
		);
		return $fields;
	}
	...
	public function fetch_lists( $args ) {
		...
		if ( ! empty( $api_response ) && is_array( $api_response ) ) {
			...
			switch ( $api_response ) {
				case '401 ' :
					return __( 'Invalid Public API key or Private API key', 'bloom' );
					break;
				case '403 Forbidden' :
					return __( 'Invalid Account ID', 'bloom' );
					break;
				default:
					return $api_response;
					break;
	...
	public function add_subscriber( $args ) {
		...
		if ( ! empty( $api_response['member_id'] ) ) {
			...
			$error_message = __( 'An error occured, please try again later', 'bloom' );
		}
