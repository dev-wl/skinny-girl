...
define( 'ET_DASHBOARD_DIR', trailingslashit( dirname(__FILE__) ) );
define( 'ET_DASHBOARD_PLUGIN_URI', plugins_url( '', __FILE__ ) );
...
class ET_Dashboard {
	...
	function __construct( $args ) {
		...
		$this->options_path = isset( $args['et_dashboard_options_path'] ) ? $args['et_dashboard_options_path'] : ET_DASHBOARD_DIR . 'includes/options.php';
	...
	public static function load_fonts_class() {
		if ( ! class_exists( 'ET_Dashboard_Fonts' ) ) {
			require_once( ET_DASHBOARD_DIR . 'includes/google_fonts.php' );
		}
					...
					foreach( $value[ 'contents' ] as $key => $value ) {
						...
						foreach( $options_array as $option) {
							...
							switch( $option[ 'type' ] ) {
								...
								case 'font_select' :
									$fonts_class = ET_Dashboard::load_fonts_class();
